...
from .types import BackendConfig, BackendList, FilterConfig, PathLike
...
class Configuration:
    ...
    def from_dict(cls, data: dict[str, Any]) -> Configuration:
        ...
        if "pdf_filters" in data:
            config.pdf_filters = [
                FilterConfig(**f) for f in data["pdf_filters"]
            ]
        if "svg_filters" in data:
            config.svg_filters = [
                FilterConfig(**f) for f in data["svg_filters"]
            ]
    ...
    def validate(self) -> None:
        ...
        if self.cache.enabled and self.cache.directory:
            if not self.cache.directory.parent.exists():
                raise ValidationError(
